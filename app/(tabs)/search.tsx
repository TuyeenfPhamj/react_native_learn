      ...
      <FlatList 
        ...
        ListHeaderComponent={
          <>
            ...
            {!loading && !error && searchQuery.trim() && movies ?.length > 0 && ( <Text className='text-xl text-white font-bold'>Search Result for{` `} <Text className='text-purple-300'>{searchQuery}</Text></Text>)}
          </>
        }
        ListEmptyComponent={
          !loading && !error ? (
